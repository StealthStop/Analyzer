...
class LeadingJetCut: public Cut{ 
    public:
        LeadingJetCut(): Cut("LeadingJetCut", false,{"LeadingJetsPass", "NotLeadingJetsPass" }){}
        void calculate(const SliceData& data)  override{
            auto& Jets = data.Jets;
...
class SigCut: public Cut{ 
    public:
        SigCut(): Cut("SigCut", false,{"Sig", "NotSig" }){}
        void calculate(const SliceData& data)  override{
            auto& Jets = data.Jets;
            auto& nsr21 = data.nsr21;
            ...
            std::size_t j1_index =  std::distance(Jets.begin(), firstLargestPt);
            std::size_t j2_index =  std::distance(Jets.begin(), secondLargestPt);
            if (std::size(Jets)>1 && nsr43[j1_index] < 0.8 
                    && nsr43[j2_index] <0.8 
                    && nsr42[j1_index] < 0.5 
                    && nsr42[j2_index] < 0.5
...
Analyze2W::Analyze2W() {
    my_histos.cuts.push_back(std::make_unique<HTCut>());
    my_histos.cuts.push_back( std::make_unique<LeadingJetCut>());
    my_histos.cuts.push_back( std::make_unique<SigCut>());
    my_histos.constructChains({{"HTCut", "LeadingJetCut", "SigCut"},{"HTCut"}});
    InitHistos();
...
void Analyze2W::InitHistos() {
    ...
    my_histos.createNewHistogram("NSubRatio43", 100, 0, 2,  "NSR 42");
    my_histos.createNewHistogram("NSubRatio42", 100, 0, 2,  "NSR 43");
    ...
    my_histos.createNewHistogram("JetPt", 50, 0, 500,    "jet pt (goodjets_pt20)", "", true);
...
#define MAKE_RATIO(x,y) auto nsr##x##y = computeRatio(JetsCA12_NsubjettinessTau##x,JetsCA12_NsubjettinessTau##y)
        ...
        SliceData data{HT_trigger_pt30, NJets, JetsCA12, nsr21, nsr42, nsr43};
...
void Analyze2W::WriteHistos(TFile *outfile) {
    ...
    for ( auto & h : my_histos) {
        if(h.first.find("WPt_") == std::string::npos) continue;
