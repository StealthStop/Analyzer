...
void Analyze1Lep::Loop(NTupleReader& tr, double, int maxevents, bool)
{
    while( tr.getNextEvent() )
    {
        ...
        const auto& MblVec                    = tr.getVec<float>("MblVec");
        const auto& passBlind                 = tr.getVar<bool>("passBlindLep_Good");            
        const auto& deepESM_val               = tr.getVar<float>("deepESM_val");
        const auto& deepESM_valNonIsoMuon     = tr.getVar<float>("deepESM_valNonIsoMuon");
