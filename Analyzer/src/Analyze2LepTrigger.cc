#define Analyze2LepTrigger_cxx
#include "Analyzer/Analyzer/include/Analyze2LepTrigger.h"
#include "Framework/Framework/include/Utility.h"
#include "NTupleReader/include/NTupleReader.h"

#include <iostream>

Analyze2LepTrigger::Analyze2LepTrigger()
{
    InitHistos();
}

//Define all your histograms here. 
void Analyze2LepTrigger::InitHistos()
{
    TH1::SetDefaultSumw2();
    TH2::SetDefaultSumw2();
    ...
    Double_t ptBinEdges[ nPtBins + 1 ] = { 30.0, 60.0, 100.0, 200.0 };
...
void Analyze2LepTrigger::Loop(NTupleReader& tr, double, int maxevents, bool)
{
    while( tr.getNextEvent() )
    {
        ...
        const auto& passBaseline        = tr.getVar<bool>("passBaselineGoodOffline2l");
...
void Analyze2LepTrigger::WriteHistos(TFile* outfile)
{
    outfile->cd();
...
bool Analyze2LepTrigger::containsGoodLepton( const std::vector<utility::LorentzVector>& leptons, const std::vector<bool>& goodLeptons, double ptThreshold, double etaSelection) { 
...
int Analyze2LepTrigger::goodLeptonIndex( const std::vector<utility::LorentzVector>& leptons, const std::vector<bool>& goodLeptons) {
    for( unsigned int iLep = 0; iLep < leptons.size(); ++iLep ) {
        if( !goodLeptons.at( iLep ) ) continue;
...
void Analyze2LepTrigger::fillHistos( const std::map<std::string, bool>& cutMap, bool passLeptonTriggers, const utility::LorentzVector& lepton, double theWeight ) {
    for( auto& kv : cutMap ) {
        if( kv.second ) {
...
void Analyze2LepTrigger::printTriggerList( const std::vector<std::string>& TriggerNames ) {
