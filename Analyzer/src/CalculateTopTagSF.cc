...
void CalculateTopTagSF::InitHistos(const std::string& histoFileTag)
{
    ...
    const std::vector<double> ptBinsMrg  = { 400, 600, 1000 };
        ...
        if( passMadHT )
        {
            ...
            {                
                ...
                if ( genTop )
                {
                    ...
                    {
                        my_2d_histos["d_eff_res_"+filetag]->Fill( top->P().Pt(), top->P().Eta(), eventweight );
                        if( top->getDiscriminator() > resolvedWP ) my_2d_histos["n_eff_res_"+filetag]->Fill( top->P().Pt(), top->P().Eta(), eventweight ); 
                    }
                ...
                else
                {
                    if ( top->getType() == TopObject::RESOLVED_TOP )
                    {
                        my_2d_histos["d_mis_res_"+filetag]->Fill( top->P().Pt(), top->P().Eta(), eventweight );
                        if( top->getDiscriminator() > resolvedWP ) my_2d_histos["n_mis_res_"+filetag]->Fill( top->P().Pt(), top->P().Eta(), eventweight ); 
                    }
                    else if ( top->getType() == TopObject::MERGED_TOP )
                    {
                        my_2d_histos["d_mis_mrg_"+filetag]->Fill( top->P().Pt(), top->P().Eta(), eventweight );
                        if( top->getDiscriminator() > mergedWP ) my_2d_histos["n_mis_mrg_"+filetag]->Fill( top->P().Pt(), top->P().Eta(), eventweight ); 
                    }
                }
            }
        }
