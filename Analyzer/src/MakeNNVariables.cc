...
void MakeNNVariables::Loop(NTupleReader& tr, double, int maxevents, bool)
{
    ...
    while( tr.getNextEvent() )
    {
        ...
        // Begin loop over suffixes to fill trees
        for(const auto& myVarSuffix : my_var_suffix)
        {
            ...
            if (!treeInit[myVarSuffix])
            {
                // Get any intialization out of the way first, and do just once
                for (const auto& channel : my_channels)
                {
                    ...
                    for (const auto& split : my_splits)
                    {
                        ...
                        if (split != "count")
                        {
                            ...
                            myMiniTuple[split][channel][myVarSuffix]->setTupleVars(varGeneral);
                            myMiniTuple[split][channel][myVarSuffix]->setTupleVars(varEventShape); 
