...
import ROOT, random, os, argparse, string
...
class StackPlotter:
    ...
    def __init__(self, approved, noRatio, printNEvents, year, outpath, inpath, normMC, histograms, backgrounds, signals, data):
    ...
    def makeLegends(self):
        ...
        nColumns = 1   if self.printNEvents else 3
        shift    = 0.2 if self.printNEvents else 0.1

        bkgXmin = 0.60;                 bkgYmin = (1.0-(self.TopMargin+shift)/self.upperSplit)
        bkgXmax = 1.0-self.RightMargin; bkgYmax = 1.0-(self.TopMargin/self.upperSplit)

        bkgLegend = ROOT.TLegend(bkgXmin, bkgYmin, bkgXmax, bkgYmax)
        bkgLegend.SetTextSize(textSize)
        bkgLegend.SetNColumns(nColumns)

        sigXmin = self.LeftMargin if self.printNEvents else 0.3; sigYmin = (1.0-(self.TopMargin+0.1)/self.upperSplit)
        sigXmax = bkgXmin;                                      sigYmax = bkgYmax

        sigLegend = ROOT.TLegend(sigXmin, sigYmin, sigXmax, sigYmax)
        sigLegend.SetTextSize(textSize)

        return bkgLegend, sigLegend
    ...
    def makePlots(self):
        ...
        for hname, hinfo in self.histograms.items():
            ...
            newName = hname
            for i in orders:
                newName = hname.replace("@", "%d"%(i))
                hinfo["X"]["title"] = hinfo["X"]["title"].replace("@", "%d"%(i))

                canvas               = self.makeCanvas(hinfo["logY"])
                bkgLegend, sigLegend = self.makeLegends()
                ...
                if not self.noRatio:
                    ...
                    hinfo["Y"]["title"] = "1 - #frac{Pred.}{Obs.}"
                    hinfo["X"]["rebin"] = 1

                    canvas.cd(2)
                    ROOT.gPad.SetGridy()
                    ratio = Histogram(ratio, None, self.upperSplit/self.scale, self.lowerSplit/self.scale, None, hinfo, rinfo, 0.8).histogram
...
if __name__ == "__main__":
    ...
    plotter = StackPlotter(args.approved, args.noRatio, args.printNEvents, args.year, args.outpath, args.inpath, args.normMC, histograms, backgrounds, signals, data)
    plotter.makePlots()
